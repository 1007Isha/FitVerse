...
const WorkoutDetails = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        'https://fitverse-eaj1.onrender.com/api/workouts',
        {
          ...workoutData,
  ...
  const fetchPastWorkouts = async () => {
    try {
      const response = await axios.get('https://fitverse-eaj1.onrender.com/api/workouts', {
        headers: { Authorization: `Bearer ${localStorage.getItem('jwttoken')}` },
      });
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      const response = await axios.get(`https://fitverse-eaj1.onrender.com/api/workouts/analytics?exercise=${analyticsExercise}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('jwttoken')}` },
      });
